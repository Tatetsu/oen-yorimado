...
> **2026-04（後半）の仕様変更**: フォームから「記録日」項目を廃止したことに伴い、自動送信の判定基準を「前日の記録日」→「過去24時間以内のタイムスタンプ＋未送信」に変更。手動送信も「指定日の記録日」→「指定日のタイムスタンプ日付」基準へ変更。連泊中の毎朝送信は、各夜の翌朝に1件ずつ送信されたフォームが過去24時間ウィンドウで拾われることで実現される。
>
