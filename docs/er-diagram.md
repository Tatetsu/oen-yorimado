...
| 利用日 | B | 日付 | ✓ | 連泊は宿泊日ごとに展開（旧名: 記録日） |
| スタッフ1 | C | テキスト | | 振り分けは固定スタッフ |
| スタッフ2 | D | テキスト | | 任意。満枠日は固定スタッフで補完 |
| 児童名 | E | テキスト | ✓ | |
| 入所日時 | F | 日時 | | ペアリング後の論理1宿泊の値。振り分けは補完値 |
| 退所予定日時 | G | 日時 | | ペアリング後の論理1宿泊の値。振り分けは補完値 |
| 送迎(往) | H | 数値 | | 利用日==入所日 → 1 |
...
| 連泊 | V | boolean | | 連泊1宿泊から展開された行はtrue |
| 宿泊PK | W | テキスト | | 児童名|入所日（yyyy-MM-dd）。デフォルト非表示 |
